...
typedef struct {
	...
	//	该成员保存着在文件的程序头表（program header table）中一个entry的大小
	//	(以字节计数)。所有的entry都是同样的大小。

	Elf32_Half          e_phnum;
	//	该成员保存着在程序头表中entry的个数。
	//	因此，e_phentsize和e_phnum的乘积就是表的大小(以字节计数).
	//	假如没有程序头表（program header table），e_phnum变量为0。

	Elf32_Half          e_shentsize;
	//	该成员保存着section头的大小(以字节计数)。
	//	一个section头是在section头表(section header table)的一个entry；
	//	所有的entry都是同样的大小。

	Elf32_Half          e_shnum;
	//	该成员保存着在section header table中的entry数目。
